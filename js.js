...
$(document).ready(function () {
    ...
    $('#formularioStock').on('change', "#marcas", function () {
        $('#marcas option:selected').each(function () {
            ...
            var yearID = $('#year').val();
            if (marcasID) {
                $.post("ajaxData.php", {refMarca: marcasID, refYear: yearID}, function (data) {
                    $('#modelos').html(data);
                });
            } else {
                $('#modelos').html('<option value="">Selecciona una marca primero</option>');
            }
        });
    });
    $('#formularioStock').on('change', '#year', function () {
        $('#year option:selected').each(function () {
            ...
            if (yearID) {
                $.post("ajaxData.php", {refYear: yearID, refMarca: marcasID}, function (data) {
                    $('#modelos').html(data);
                });
            } else {
                $('#modelos').html('<option value="">Selecciona una marca primero</option>');
    ...
    $('#color').on('change', function () {
        $('#color option:selected').each(function () {
            ...
            var tipoID = $('#tipo').val();
            if (colorID) {
                $.post("ajaxData.php", {refModelo: modelID, refTipo: tipoID, refColor: colorID}, function (data) {
                    $('#stock').html(data);
                });
            } else {
    ...
    $("#GrupoRelacionados").on("click", ".eliminarRelaciones .eliminarRelacion", function () {
            ...
            body: 'idModeloEditar=' + encodeURIComponent(idModelo)
                ...
                .then(data => {
                    if (data.error) {
                        ...
                        var myModal = new bootstrap.Modal(document.getElementById('editarModeloModal'));
                        myModal.show();
                    }
    ...
    $("#tablastock").on("click", ".ordenar", function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $("#" + id + ".ordenar").after('  <div class="spinner-border text-info" style="width:14px; height:14px" role="status"><span class="visually-hidden">Loading...</span></div>');
            },
            success: function (data) {
                ...
                if (ascdesc == "ASC") {
                    $('#' + id + ".ordenar").append('<i class="bi bi-arrow-down-circle"></i>')
                } else if (ascdesc == "DESC") {
                    $('#' + id + ".ordenar").append('<i class="bi bi-arrow-up-circle"></i>')
                }
            }
        });
    });
    ...
    $("#tablastock").on('change', '#filterModel', function () {
        $('#filterModel option:selected').each(function () {
            ...
            $.ajax({
                ...
                data: {'parametro': id, 'ascdesc': ascdesc, 'page': page, 'model': model},
                beforeSend: function () {
                    $("#filterModel").after('  <div class="spinner-border text-info" style="width:14px; height:14px" role="status"><span class="visually-hidden">Loading...</span></div>');
                },
                success: function (data) {
                    /*console.log(id + " " + ascdesc + " " + page + " " + model);*/
                    $('#tablastock').html(data);
                    $("#filterModel").after("<form action='functions.php' method ='post'><input type ='hidden' name='model' value =" + model + "><button type='submit' class='btn btn-primary' name='exportarByModel'>Exportar este modelo<i class='bi bi-arrow-right-short'></i></button></form>")
                    $('#' + id + ".ordenar").addClass('active');
                }
            });
        });
    ...
    $("#modelos-page").on("click", ".order", function () {
        ...
        $.ajax({
            ...
            beforeSend: function () {
                $("#" + id + ".order").after('<div class="spinner-border text-info" style="width:14px; height:14px" role="status"><span class="visually-hidden">Loading...</span></div>');
            },
            success: function (data) {
                ...
                if (ascdesc == "ASC") {
                    $('#' + id + ".order").append('<i class="bi bi-arrow-down-circle"></i>')
                } else if (ascdesc == "DESC") {
                    $('#' + id + ".order").append('<i class="bi bi-arrow-up-circle"></i>')
                }
            }
        });
    });
